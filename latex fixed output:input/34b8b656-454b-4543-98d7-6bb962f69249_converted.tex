...
\title{GlucoBench: Curated List of Continuous Monitoring Datasets with Prediction Benchmarks }

\author{Renat Sergazinov ${ }^{1, *}$\\
ElizabethChun ${ }^{1}$, Valeriya Rogovchenko ${ }^{1}$, Nathaniel Fernandes ${ }^{2}$, Nicholas Kasman ${ }^{2}$, Irina Gaynanova ${ }^{1 *}$\\
...
\begin{center}
  \includegraphics[width=\textwidth]{2025_11_10_aeeab37f03cd8373f6afg-2}
...
for dynamic.\\
Data splitting. Each dataset is split into train, validation, and in-distribution (ID) test sets using $90 \% 10 \% \mathcal{D}=\mathcal{D}_{t r} \cup \mathcal{D}_{v a l} \cup \mathcal{D}_{i d} \cup \mathcal{D}_{o d}$
...
$\mathrm{D}_{t r}$, andthenthesamevaluesareusedtorescale(3)D ${ }_{v a l}, \mathcal{D}_{i d}$, and $\mathcal{D}_{o d}$.\\
Input-output pairs. Let $\mathbf{x}_{j, k: k+L}^{(i)}$ be a length $L$ contiguous slice of a segment from index $k$ to $k+L$. We define an input-output pair as
...
and Tisthelengthofpredictioninterval.Ourchoicesof(6)T, Landk\\
areasfollows.Inlinewiththepreviousworks(Oviedoetal., 2017)wef

\begin{itemize}
  \item houraheadforecasting(
\end{itemize}
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_11_10_aeeab37f03cd8373f6afg-4}
\captionsetup{labelformat=empty}
...
$\mathrm{T}=12$ for 5 minutefrequency).Wetreat L\\
asahyper\\
-parameter formodeloptimizationsincedif ferentmodelsha

\begin{itemize}
  \item termdependencies.W esample\\
k
\end{itemize}
...
\begin{center}
\begin{tabular}{ccccr}
Task & \multicolumn{1}{c}{1:} & \multicolumn{2}{c}{Predictive} & Accuracy. \\
Given & the & model & prediction & (9) \\
\end{tabular}
\end{center}
...
Task 2: Uncertainty Quantification. To measure the quality of uncertainty quantification, we use two metrics: loglikelihood on test data and calibration. For models that estimate a parametric predictive distribution over the future values, $\hat{P}_{j, k+L+1: k+L+T}^{(i)}: \mathbb{R}^{T} \rightarrow[0,1]$, we evaluate log-likelihood as
...
\begin{figure}[h]
\begin{center}
  \includegraphics[width=\textwidth]{2025_11_10_aeeab37f03cd8373f6afg-5}
\captionsetup{labelformat=empty}
\caption{Fig. 3: Figure 3: Example data processing on Weinstock et al. (2016). The red arrows denote segmentation, green blocks interpolate values, and red brackets indicate dropped segments due to length.}
\end{center}
\end{figure}

confidence levels $0<p_{1}<\cdots<p_{M}<1$; (2) estimate realized confidence level $\hat{p}_{m}$ using $N$ test input-output pairs as

\[
\hat{p}_{m}=\frac{\left|\left\{y_{j, k+L+t}^{(i)} \mid \hat{F}_{j, k+L+t}^{(i)}\left(y_{j, k+L+t}^{(i)}\right) \leq p_{m}\right\}\right|}{N}(11)
\]

and (3) compute calibration across all $M$ levels as\\
$C a l_{t}=\sum_{1}^{M}\left(p_{m}-\hat{p}_{m}\right)^{2}(12)$

\subsection*{4.2 Models}
To benchmark the performance on the two tasks, we compare the following models. ARIMA is a classical time -series model, which has been previously used for glucose predictions (Otoom et al., 2015; Yang et al., 2019). Linear regression is a simple baseline with a separate model for each time step\\
$\mathrm{t}=1, \ldots$, T.XGBoost(Chen\&Guestrin, 2016)isgradient boostedtreemethod, withaseparatemodel foreachtimestepttosupportmulti-outputregression.Transformerrepresentsastandardencoder-decoderauto-regressiveTransformerimplementation(Vaswanietal., 2017).TemporalFusionTransformer(TFT)isaquantilebasedmodelthatusesRNNwithattention.TFTistheonlymodelthatof fersout-of-the-boxsupportforstatic, dynamicknown, anddynamicunknowncovariates.NHiT Susesneuralhierarchicalinterpolationfor sourceDARTSlibrary(Herzenetal., 2022).ForLatentODEandGluformer, weusetheimplementationinPyTorch(Rubanovaetal.,
